...
public class BaiHatDAO {
    ...
    String sql_selectLinkNhac = "select AmThanh from BaiHat where MaBH = ?";
...
//    String sql_selectByBaiHat = "SELECT * FROM BaiHat WHERE TenBH COLLATE Latin1_General_CI_AI LIKE N'%" + keyword + "%'";
    ...
    public BaiHatEntity selectByLinkAmThanh(String maBH) {
        List<BaiHatEntity> list = this.selectBySql(sql_selectById, maBH);
        if (list.isEmpty()) {
            return null;
        }
        return list.get(0);
